...
void renew_lease(const char *interface, struct dhcp_message *ack_msg, int send_socket, int recv_socket, struct sockaddr_in *server_addr) {
    // Definimos la estructura que va a almacenar la ip para imprimir mensaje.
    struct in_addr addr;
    addr.s_addr = ack_msg->yiaddr;
    
    // Declaramos server_len como socklen_t para utilizarla en receive_message
    ...
    if (send_message(send_socket, &request_msg, server_addr, sizeof(*server_addr)) < 0) {
        exit(EXIT_FAILURE);
    }
...
void *lease_renewal(void *arg) {
    ...
    while (i<=3) {
        ...
        if (time_elapsed >= renew_threshold) {
            ...
            renew_lease(args->interface, args->ack_msg, args->send_socket, args->recv_socket, &args->server_addr);
...
void send_dhcp_release(int send_socket, uint32_t assigned_ip, uint32_t server_ip, uint32_t transaction_id, uint8_t *chaddr, struct sockaddr_in *server_addr, socklen_t server_len) {
    ...
    if (send_message(send_socket, &release_msg, server_addr, server_len) < 0) {
