...
int send_message(int fd, struct dhcp_message *msg, struct sockaddr_in *server_addr, socklen_t server_len) {
    // Enviamos el mensaje de broadcast al puerto del server.
    if (sendto(fd, msg, sizeof(struct dhcp_message), 0, (struct sockaddr *)server_addr, server_len) < 0) {
